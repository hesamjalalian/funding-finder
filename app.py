...
racialized_group = st.multiselect(
    ...
    [
        ...
        "Latin American", "Southeast Asian", "West Asian", "Korean", "Japanese",
        "Another racialized population group", "Women / Girls", "Youth (Ages 16-29)",
        "Seniors (Ages 65+)", "Low-Income / Economically Disadvantaged Individuals",
        "Newcomers / Immigrants / Refugees", "None of the above / Not applicable"
    ]
)
...
team_members = st.multiselect(
    ...
    ["Solo entrepreneur", "2 to 5", "5 to 10", "10 to 50", "More than 50", "Other (please specify)"]
)
...
founder_education = st.selectbox(
    "Founder's Education",
    ["High School", "Bachelor's Degree", "Master's Degree", "PhD", "Trade Certification"]
)
...
if st.button("Generate Funding Report"):
    if not city or not country:
        ...
        try:
            ...
            # Streamlit download
            st.download_button(
                label="Download Funding Report as PDF",
                data=buffer,
            ...
            # Save automatically to Desktop
            desktop_report_path = os.path.join(os.path.expanduser("~"), "Desktop", "funding_report.pdf")
            with open(desktop_report_path, "wb") as f:
                f.write(buffer.getbuffer())
            st.success("Funding report automatically saved to Desktop as funding_report.pdf")
